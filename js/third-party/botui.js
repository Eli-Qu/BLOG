...
function bot_ui_ini() {
    ...
    botui.message.add({
        ...
        botui.message.add({
            ...
            botui.message.add({
                delay: 1100,
                content: "ummm...maybe a lovely girl!"
            }).then(function() {
                botui.action.button({
                    delay: 1600,
                    action: [{
    ...
    var sure = function() {
        ...
        secondpart = function() {
            botui.message.add({
                delay: 1500,
                content: "就读于重庆交通大��}).then(function(){botui.message.add({delay:1500,content:"
                是在计算机专业学物联网工程的程序��
            }).then(function() {
                botui.message.add({
                    delay: 1200,
                    content: "将敲代码看成��种快��}).then(function(){botui.message.add({delay:1500,content:"
                    主修 Java�� Python,
                    略知 linux,
                    偶尔也搞 HTML / CSS / JavaScript / Spring "}).then(function(){botui.message.add({delay:1500,content:"
                    专业方向， 物联网及智能家�� Internet of things) 及机器视�� machine version)
                "}).then(function(){botui.message.add({delay:1800,content:"
                喜欢看韩剧和玩手��
            }).then(function() {
                botui.action.button({
                    delay: 1100,
        ...
        botui.message.add({
                delay: 1e3,
                content: "cungudafa是村姑大发的拼音，大发在韩语发音中时很厉害的意���}).then(function(){botui.action.button({delay:1500,action:[{text:"
                为什么叫村姑呢？ ",value:"
                why - cat "}]}).then(function(a){fourthpart()})})},fourthpart=function(){botui.message.add({delay:1e3,content:"
                我的游戏名叫村姑,
                后来也有人叫我村姑学�� "}).then(function(){botui.message.add({delay:1100,content:"
                希望自己打游戏很厉害吧， 哈哈��
            }).then(function() {
                    botui.action.button({
                            delay: 1500,
                            action: [{
                                text: "域名有什么含义吗��,value:"
                                why - domain "}]}).then(function(a){fifthpart()})})})},fifthpart=function(){botui.message.add({delay:1e3,content:"
                                emmmm,
                                csdn,
                                github,
                                都是cungudafa��
